...
DOFBOT_CONFIG = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        usd_path="/home/baai/Projects/C/Dexhand/Dexhand_single/assets/limit_1.57/Assem_DexCo_2/Assem_DexCo_2/Assem_DexCo_2.usd",
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    while simulation_app.is_running():
        ...
        if count >= len(run_simulator.df):  # 运行完所有数据点后停止并画图
            ...
            plt.tight_layout()
            plt.savefig('angle_comparison.png', dpi=300, bbox_inches='tight')
            plt.show()
            
            print("角度对比图已保存为 angle_comparison.png")
